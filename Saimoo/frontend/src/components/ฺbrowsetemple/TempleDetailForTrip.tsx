...
const TempleDetailForTrip = () => {
  ...
  return (
    <div className="bg-white min-h-screen p-6">
      ...
      <p className="text-gray-600 mt-4">{temple.description || "ไม่มีคำอธิบาย"}</p>
      <p className="text-gray-500 text-sm mt-2">จังหวัด: {temple.province}</p>
      <p className="text-gray-500 text-sm">ถูกใจ: {temple.like}</p>
      <button className="w-full h-5 bg-gray-200 flex items-right justify-center rounded-lg mt-4"> เลือกวัด </button>
    </div>
  );
};

export default TempleDetailForTrip;
